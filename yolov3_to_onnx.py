...
class DarkNetParser(object):
    ...
    def parse_cfg_file(self, cfg_file_path):
        ...
        """
        #print('cfg_file_path : ', cfg_file_path);   exit()
        #with open(cfg_file_path, 'r') as cfg_file:
        with open(cfg_file_path, 'rb') as cfg_file:
            #remainder = cfg_file.read()
            remainder = cfg_file.read().decode('utf-8')
            while remainder is not None:
                #print('remainder : ', remainder);   exit()
    ...
    def _parse_params(self, param_line):
        ...
        #print('param_line : ', param_line); #exit()
...
class GraphBuilderONNX(object):
    ...
    def build_onnx_graph(
        ...
        for layer_name in layer_configs.keys():
            ...
            #print('1 layer_dict.keys() : ', layer_dict.keys()); exit()
    ...
    def _make_onnx_node(self, layer_name, layer_dict):
        ...
        """
        #print('layer_dict : ', layer_dict); exit()
        layer_type = layer_dict['type']
        if self.input_tensor is None:
            #print('layer_type : ', layer_type); #exit()
            if layer_type == 'net':
                #print('what the'); exit()
...
def generate_md5_checksum(local_path):
    ...
    #with open(local_path, encoding='ISO-8859-1' if is_py3 else 'utf-8') as local_file:
    #with codecs.open(local_path, encoding = 'ISO-8859-1' if is_py3 else 'utf-8') as local_file:
        data = local_file.read()
        #return hashlib.md5(data.encode('utf-8')).hexdigest()
        return hashlib.md5(data).hexdigest()

def get_checksum_yolo(verzion, is_weight):
    cs = None
    if 'v3-tiny' == verzion:
        if is_weight:
            cs = '3bcd6b390912c18924b46b26a9e7ff53'
        else:
            cs = '146ad314a56caff08583d09d82b26a6f'
    elif 'v3' == verzion:
        if is_weight:
            cs = 'c84e5b99d0e52cd466ae710cadf6d84c'
        else:
            cs = 'b969a43a848bbf26901643b833cfb96c'
    return cs
...
def download_file(local_path, link, checksum_reference=None):
    ...
    if checksum_reference is not None:
        ...
        #print('checksum : ', checksum); exit()
        if checksum != checksum_reference:
            raise ValueError(
                'The MD5 checksum %s of local file %s differs from %s, please manually remove \
...
def main():
    ...
    cfg_file_path = download_file(
        ...
        cfg_local_path,
        #url_cfg,
        'https://raw.githubusercontent.com/pjreddie/darknet/master/cfg/yolo{}.cfg'.format(v_yolo)
    ...
    #print('layer_configs["000_net"].keys() : ', layer_configs['000_net'].keys()); exit()
    ...
    weights_file_path = download_file(
        #'yolov3.weights',
        weight_local_path,
        'https://pjreddie.com/media/files/yolo{}.weights'.format(v_yolo)
